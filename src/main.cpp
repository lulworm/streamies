...
#if defined(NDEBUG)
#error "STRMS cannot be compiled without assertions."
#endif
...
bool ContextualCheckZerocoinSpend(const CTransaction& tx, const libzerocoin::CoinSpend* spend, CBlockIndex* pindex, const uint256& hashBlock)
{
    ...
    if (IsSerialInBlockchain(spend->getCoinSerialNumber(), nHeightTx))
        return error("%s : zSTRMS spend with serial %s is already in block %d\n", __func__,
...
{
    //Check to see if the zSTRMS is properly signed
    if (pindex->nHeight >= Params().Zerocoin_Block_V2_Start()) {
        try {
            if (!spend->HasValidSignature())
                return error("%s: V2 zSTRMS spend does not have a valid signature\n", __func__);
        ...
        if (spend->getSpendType() != expectedType) {
            return error("%s: trying to spend zSTRMS without the correct spend type. txid=%s\n", __func__,
                         tx.GetHash().GetHex());
        }
    }
    ...
    if (pindex->nHeight >= Params().Zerocoin_Block_Public_Spend_Enabled()) {
        ...
        if (v1Serial) {
            return error("%s : zSTRMS v1 serial spend not spendable, serial %s, tx %s\n", __func__,
    ...
    if (!spend->HasValidSerial(Params().Zerocoin_Params(v1Serial)))  {
        ...
        if (!isBlockBetweenFakeSerialAttackRange(pindex->nHeight))
            return error("%s : zSTRMS spend with serial %s from tx %s is not in valid range\n", __func__,
                     spend->getCoinSerialNumber().GetHex(), tx.GetHash().GetHex());
        else
...

    {
        ...
        if (tx.HasZerocoinSpendInputs()) {
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : zSTRMS spend tx %s already in block %d",
            ...
            // Check that zSTRMS mints (if included) are not already known
...
int64_t GetBlockValue(int nHeight)
{
    ...
    // Don't sync this code, it's specified by AEX
    if (nHeight == 0) {
        nSubsidy = 0 * COIN;
    } else if (nHeight == 1 ) {
        nSubsidy = 50000 * COIN;
    } else if (nHeight > 1 && nHeight <= 3500) {
        nSubsidy = 0.35 * COIN;
	} else if (nHeight > 3500 && nHeight <= 24150) {
        nSubsidy = 0.4 * COIN;
	} else if (nHeight > 24150 && nHeight <= 45400) {
        nSubsidy = 0.45 * COIN;
	} else if (nHeight > 45400 && nHeight <= 61860) {
        nSubsidy = 0.5 * COIN;
	}  else if (nHeight > 61860 && nHeight <= 85640) {
        nSubsidy = 0.6 * COIN;
	}  else if (nHeight > 85640 && nHeight <= 102950) {
        nSubsidy = 0.75 * COIN;
	}  else if (nHeight > 102950 && nHeight <= 122490) {
        nSubsidy = 0.9 * COIN;
	}  else if (nHeight > 122490 && nHeight <= 166270) {
        nSubsidy = 1.05 * COIN;
	}  else if (nHeight > 166270 && nHeight <= 210600) {
        nSubsidy = 1.25 * COIN;
	}  else if (nHeight > 210600 && nHeight <= 240920) {
        nSubsidy = 1.5 * COIN;
	}  else if (nHeight > 240920 && nHeight <= 283650) {
        nSubsidy = 1.35 * COIN;
	}  else if (nHeight > 283650 && nHeight <= 385780) {
        nSubsidy = 1.2 * COIN;
    } else if (nHeight > 385780) {
        nSubsidy = 1 * COIN;
    } else {
	nSubsidy = 1 * COIN;
	}
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount, bool isZPIVStake)
{
	...
	if (nHeight < 1000) {
        ret = 0;
    } else if (nHeight <= 3500 && nHeight >= 1000) {
		ret = blockValue * 0.65;
	} else if (nHeight <= 24150 && nHeight >= 3500) {
		ret = blockValue * 0.7;
	} else if (nHeight <= 45400 && nHeight >= 24150) {
		ret = blockValue * 0.75;
	} else if (nHeight <= 61860 && nHeight >= 45400) {
		ret = blockValue * 0.8;
	} else if (nHeight <= 85640 && nHeight >= 61860) {
		ret = blockValue * 0.85;
	} else if (nHeight <= 102950 && nHeight >= 85640) {
		ret = blockValue * 0.90;
	} else if (nHeight > 102950) {
		ret = blockValue * 0.9;
	} else {
		ret = blockValue * 0.8;
	}
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from STRMS
...
{
    RenameThread("streamies-scriptch");
...
void RecalculateZPIVSpent()
{
    ...
    while (true) {
        ...
        //Remove spends from zSTRMS supply
...
bool RecalculatePIVSupply(int nHeightStart)
{
    ...
    uiInterface.ShowProgress(_("Recalculating STRMS supply..."), 0);
    while (true) {
        if (pindex->nHeight % 1000 == 0) {
            LogPrintf("%s : block %d...\n", __func__, pindex->nHeight);
            int percent = std::max(1, std::min(99, (int)((double)((pindex->nHeight - nHeightStart) * 100) / (chainActive.Height() - nHeightStart))));
            uiInterface.ShowProgress(_("Recalculating STRMS supply..."), percent);
        }
...
{
    // STRMS: recalculate Accumulator Checkpoints that failed to database properly
...
bool UpdateZPIVSupply(const CBlock& block, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    // A one-time event where only the zSTRMS supply was off (due to serial duplication off-chain on main net)
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    for (unsigned int i = 0; i < block.vtx.size(); i++) {
        ...
        if (tx.HasZerocoinSpendInputs()) {
            ...
            // Check that zSTRMS mints are not already known
            if (tx.HasZerocoinMintOutputs()) {
                for (auto& out : tx.vout) {
                    if (!out.IsZerocoinMint())
                        continue;
    ...
    //Track zSTRMS money supply in the block index
    if (!UpdateZPIVSupply(block, pindex, fJustCheck))
        return state.DoS(100, error("%s: Failed to calculate new zSTRMS supply for block=%s height=%d", __func__,
...
void static UpdateTip(CBlockIndex* pindexNew)
{
     ...
     *
#ifdef ENABLE_WALLET
    // If turned on AutoZeromint will automatically convert STRMS to zPIV
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    for (const CTransaction& tx : block.vtx) {
        ...
        // double check that there are no double spent zSTRMS spends in this block
        if (tx.HasZerocoinSpendInputs()) {
            for (const CTxIn& txIn : tx.vin) {
                bool isPublicSpend = txIn.IsZerocoinPublicSpend();
                if (txIn.IsZerocoinSpend() || isPublicSpend) {
                    ...
                    if (std::count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of zSTRMS serial %s in block\n Block: %s",
...
bool CheckWork(const CBlock block, CBlockIndex* const pindexPrev)
{
    ...
    if (block.nBits != nBitsRequired) {
        // Streamies Specific reference to the block with the wrong threshold was used.
        if ((block.nTime == (uint32_t) Params().StreamiesBadBlockTime()) && (block.nBits == (uint32_t) Params().StreamiesBadBlocknBits())) {
            // accept STRMS block minted with incorrect proof of work threshold
            return true;
        }
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    if (isPoS) {
        ...
        if (isBlockFromFork) {
            ...
            // Now that this loop if completed. Check if we have zSTRMS inputs.
            if(hasZPIVInputs){
...
bool static ProcessMessage(CNode* pfrom, std::string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // STRMS: We use certain sporks during IBD, so check to see if they are
